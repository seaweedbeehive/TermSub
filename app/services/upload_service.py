...
from app.models.video import Video, VideoStatus, ContentType


# Allowed file extensions
ALLOWED_VIDEO_EXTENSIONS = {
    ".mp4", ".mov", ".avi", ".mkv", ".webm", ".m4v", ".mpeg", ".mpg", ".mp3", ".wav", ".m4a", ".ogg"
}
...
ALLOWED_MIME_TYPES = {
    ...
    # Audio formats
    'audio/mpeg', 'audio/wav', 'audio/x-wav', 'audio/mp4', 'audio/m4a', 'audio/ogg', 'audio/vorbis',
    # Text formats
...
def sanitize_filename(filename: str) -> str:
    ...
    if len(safe_name) > 255:
        name_part, ext = safe_name[:250].rsplit('.', 1) if '.' in safe_name[:250] else (safe_name[:250], '')
...
def detect_mime_type(file: UploadFile) -> str:
    ...
    try:
        ...
        raise UploadValidationError(f"Failed to detect file type: {e}")
...
def validate_file_content(file: UploadFile, expected_content_type: str) -> None:
    ...
    if is_fallback:
        ...
        print(f"[Upload] Warning: python-magic not installed, skipping MIME validation for {file.filename}")
        return
    
    # Check if MIME type is in allowed list
    if mime_type not in ALLOWED_MIME_TYPES:
        # Special handling for text files which can have various MIME types
        if expected_content_type == ContentType.TEXT.value:
            # Accept text/* MIME types for text files
    ...
    if mime_type != 'application/octet-stream':
        if expected_content_type == ContentType.VIDEO.value:
            if not (mime_type.startswith('video/') or mime_type.startswith('audio/')):
                raise UploadValidationError(
                    f"File content ({mime_type}) does not match expected video/audio format"
                )
        elif expected_content_type == ContentType.TEXT.value:
            if not (mime_type.startswith('text/') or mime_type == 'application/x-subrip'):
                raise UploadValidationError(
...
async def save_uploaded_file(
    ...
    try:
        ...
        if file_path.exists():
            file_path.unlink(missing_ok=True)
        raise UploadValidationError(f"Failed to save file: {str(e)}")
    finally:
        await file.close()
