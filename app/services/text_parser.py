...
from app.models.video import Video, Segment, VideoStatus
...
def split_into_sentences(text: str) -> List[str]:
    ...
    # Matches sentence-ending punctuation followed by space and capital letter or end of string
...
def parse_text_file(video_id: str) -> Dict[str, Any]:
    ...
    with SessionLocal() as db:
        ...
        if video.status == VideoStatus.ERROR.value:
            raise RuntimeError(f"Video {video_id} is in ERROR status, aborting text parsing")
    ...
    try:
        ...
        progress_tracker.end_step(f"Text parsing complete! Created {total_sentences} segments")
