...
from app.models.video import ProcessingLog
from app.schemas.video import ProcessingLogEntry
...
class ProgressTracker:
    ...
    def __init__(self, video_id: str, db: Any = None) -> None:
    ...
    def info(self, step: str, message: str, details: str | None = None) -> None:
        """Log info message."""
        self._log("INFO", step, message, details)

    def warning(self, step: str, message: str, details: str | None = None) -> None:
        """Log warning message."""
        self._log("WARNING", step, message, details)

    def error(self, step: str, message: str, details: str | None = None) -> None:
        """Log error message."""
        self._log("ERROR", step, message, details)

    def _get_video_primitives(self) -> dict[str, Any] | None:
    ...
    def end_step(self, message: str = "") -> None:
        ...
        if self._step_start_time and self._current_step_name:
            ...
            self.info(self._current_step_name, msg, f"Elapsed time: {elapsed:.2f}s")

    def update_segment_progress(
        self, current: int, total: int, segment_text: str = ""
    ) -> None:
        ...
        if total > 0:
            ...
            if current == 1 or current == total or current % 10 == 0:
                self.info(
                    ...
                    f"Current segment text: {segment_text[:100]}..."
                    if segment_text
                    else None,
                )

    def estimate_time_remaining(self) -> str | None:
    ...
    def get_recent_logs(self, limit: int = 50) -> list[ProcessingLogEntry]:
        ...
        Returns:
            List of ProcessingLogEntry objects
        """
        with get_db_session() as db:
            logs = (
...
def get_progress_tracker(video_id: str, db: Any = None) -> ProgressTracker:
