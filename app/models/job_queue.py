...
class JobQueue(Base):
    ...
    timeout_at: Mapped[datetime | None] = mapped_column(
        DateTime, nullable=True, doc="Deadline after which job is considered timed out"
    )
    ...
    # Celery task ID (set when using Celery as the task queue)
    celery_task_id: Mapped[str | None] = mapped_column(
        String(100), nullable=True, index=True, doc="Celery AsyncResult task ID"
    )
