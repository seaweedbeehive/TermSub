...
down_revision: str | None = "79b85294de0a"
...
def upgrade() -> None:
    ...
    op.execute("DROP INDEX IF EXISTS idx_segments_video_seq")
    op.create_index(
    ...
    _ensure_check_constraint(
        "videos", "ck_videos_content_type", "content_type IN ('video', 'text')"
    )
    _ensure_check_constraint(
        "videos",
        "ck_videos_status",
    ...
    _ensure_check_constraint("terms", "ck_terms_source", "source IN ('auto', 'manual')")
    _ensure_check_constraint(
        "users", "ck_users_api_key_mode", "api_key_mode IN ('standard', 'byok')"
    )
